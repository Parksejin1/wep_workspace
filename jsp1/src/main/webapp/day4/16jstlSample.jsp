...
%>
${name}	<!-- 일반 자바변수는 애트리뷰트(el)가 아니므로 출력이 안됨 -->
<hr>
...
<c:if test="${age>=20}">
	...
	<!-- else 해당하는 태그가 없습니다. c:choose 비슷한 동작을 -->
<c:choose>
...
<!--  스크립트릿에서 for 문을 작성하는 것 대신 사용 -->
<hr>
<h4>for each</h4>
<c:forEach var="i" begin="1" end="10" varStatus="status" step="2">
<!--  for시작값은 1, 마지막값은 10, 변수명은 var로 지정  step은 증가값 기본은 1-->
...
<fmt:formatNumber  value="9999999"/> <br>  <!-- formatNumber는 바로 출력과 저장 , value에는 값을 직접 또는 el -->
   ...
   <!-- 2022-11-03 20:14:00  시간이 24시 기준-->
   <fmt:formatDate value="${datetime1 }" pattern="yyyy-MM-dd a hh:mm:ss"/><br>
   <!-- 2022-11-03 08:14:00  시간이 12시 기준 a는 오전/오후 표시-->
   
       <h3>문자열 메소드를 함수형태로 사용하기 : fn </h3>>
       (태그가 아니며 el 과 함께 사용헙니다. 자바의 문자열 메소드와 상당부분 일치) <br>
