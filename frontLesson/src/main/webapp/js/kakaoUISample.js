...
//함수정의 (화살표 함수)
	const testkako = () =>{
		//초창기 데이터 api가 XML 제공만 했으므로 이름에 XML 이있습니다.
		...
		//요청 헤더 항목을 추가 카카오api에서 정함 인증과 관련된 항목
		xhr.setRequestHeader('Authorization','KakaoAK a01d4aa807f665ca4101dbe945462529')
		//요청을 보내는 메소드
		xhr.send()
		
		let result=''
		//onload 는 응답이 오면 발생하는 이벤트 이벤트 처리함수 정의합니다.
		xhr.onload = function(){
		//요청 처리 결과에 대한 상태 코드 값은 status 로 확인
			...
			if(xhr.status ==200){
				...
				//xhr.response 는 응답으로 받은 내용 저장하는 xhr의 프로퍼티 xhr.response은 json문자열
				let $response = JSON.parse(xhr.response)
				console.log('응답받은 전체값 :')
	...
	document.getElementById('search').addEventListener('click',testkako)